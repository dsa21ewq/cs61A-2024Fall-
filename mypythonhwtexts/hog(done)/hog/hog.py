...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM
    onedigitofoppplayer=opponent_score%10
    tendigit=(opponent_score%100-onedigitofoppplayer)//10
    onedigitofplayer=player_score%10

    return max(3*abs(tendigit-onedigitofplayer),1)
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # Loop through number of rolls from 1 to 10
    for num_rolls in range(1, 11):
        # Create an averaged version of roll_dice
        averaged_roll = make_averaged(roll_dice, times_called)
        # Compute the average score for the current number of rolls
        score = averaged_roll(num_rolls, dice)

        # Update max_score and best_num_rolls if current score is greater
        if score > max_score:
            max_score = score
